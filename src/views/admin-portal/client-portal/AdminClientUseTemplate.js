...
import {
    ...
    adminTemplateUseForEnvelope
} from "../../../services/AdminService";

function AdminClientUseTemplate(props) {
        ...
        if (useTemplateId > 0) {
            if (selectedTemplateList.length > 0) {
                ...
                if (docIds.length > 0 || dataIds.length > 0) {
                    ...
                    adminTemplateUseForEnvelope(obj)
                        .then(response => {
                            clsUseTemplateRef?.current.click();
                            setLoading(false);
                            navigate("/back-admin/client-portal/" + client + "/envelope/edit/" + response.data.uuid + "/4/direct");
                        })
                        .catch(err => {
                            setLoading(false);
                            toast.error(Utils.getErrorMessage(err));
...
export default AdminClientUseTemplate;