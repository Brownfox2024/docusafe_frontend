...
import {
    ...
    adminModifyEnvelopeDocument
} from "../../../../../../../services/AdminService";

function AdminManageDocument(props) {
    ...
    }, [props?.docData, props.recipientsList]);
    ...
    async function onStoreDoc(e, type) {
        ...
        formData.append('envelope_id', props.envelopeId);
        ...
        if (isCall === true) {
            adminModifyEnvelopeDocument(client, formData)
                .then(response => {
                    if (type === 1) {
                        clsButtonRef.current?.click();
                        clearDocumentForm();
                    ...
                    let dList = [...props.documentList];
                    if (docId > 0) {
                        let index = dList.findIndex(x => parseInt(x.id) === parseInt(docId));
                        if (index > -1) {
                            dList[index]['name'] = docName;
                            dList[index]['request_id'] = recipientIds;
                            dList[index]['request_display'] = requestDisplay;
                            dList[index]['doc_detail'] = docDetail;
                            dList[index]['documents'] = documentsList;
                            dList[index]['date_format'] = dateFormat;
                        }
                    } else {
                        dList.push({
                            id: response.data.id,
