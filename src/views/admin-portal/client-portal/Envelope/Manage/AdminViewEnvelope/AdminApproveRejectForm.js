...
import {
    adminDataEnterUpdate,
    adminEnvelopeDownloadData,
    adminGetEnvelopeDocumentCheckStorage,
    adminInformationQuestionUpdate,
    adminManageEnvelopeDocFormStatusUpdate
...
function AdminApproveRejectForm(props) {
    ...
    const handleInformationAnswerSubmit = async (e) => {
        ...
        if (isCall === true) {
            adminInformationQuestionUpdate(client, formData)
                .then(response => {
                    let data = {...props?.formDetail};
                    data.is_call = true;
                                ...
                                {props?.formDetail?.form_detail?.questions && props?.formDetail?.form_detail?.questions.map((item, index) =>
                                    <div key={index} className="mb-4">
                                        ...
                                        {item.type === 7 && (
                                            <>
                                                <div className="mb-2"
                                                     dangerouslySetInnerHTML={{__html: item.que_comment}}/>
                                                {/* <input type="text" value={item.answer}
                                                       onChange={(e) => handleQuestionAnswer(e, index)}
                                                       className="form-control"/> */}
                                            </>
...
export default AdminApproveRejectForm;