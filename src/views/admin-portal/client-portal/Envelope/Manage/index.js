...
import AdminCheckEnvelopeCredit from "./CheckEnvelopeCredit";
import {
    adminCheckEnvelopeCredit, adminCheckEnvelopeStorage,
    adminGetEnvelopeSenderList,
    adminGetManageEnvelopeList
} from "../../../../../services/AdminService";
import CheckEnvelopeStorage from "./CheckEnvelopeStorage";

const lengths = Utils.tableShowLengths();

function AdminManageEnvelope() {
    ...
    const handleManageView = (e, data, recipient) => {
        if (parseInt(data.is_credit) === 0) {
            setLoading(true);
            adminCheckEnvelopeCredit({client_id: client})
                .then(response => {
                    setLoading(false);
            ...
            setLoading(true);
            adminCheckEnvelopeStorage({envelope_id: data.id, client_id: client})
                .then(response => {
                    setLoading(false);
        ...
        if (parseInt(data.is_credit) === 0) {
            ...
            adminCheckEnvelopeCredit({client_id: client})
                .then(response => {
                    setLoading(false);
                    if (response.data.is_credit === true) {
                        navigate('/back-admin/client-portal/' + client + '/manage/' + data.uuid + '/' + type, {
                            state: {
                                recipient_id: recipient.id,
                                recipient_name: recipient.first_name + ' ' + recipient.last_name
                            }
    ...
    return (
        <>
                ...
                <div className="custom_container">
                    ...
                    <div className="table_header_wrap">
                        <div className="nav nav-tabs wrap_left flexWrap" id="nav-tab" role="tablist">
                            ...
                            <button className="nav-link mb-3"
                                    onClick={(e) => navigate('/back-admin/client-portal/' + client + '/manage/completed')}
                                    type="button" role="tab" aria-selected="true">
            ...
            <AdminCheckEnvelopeCredit/>
            <CheckEnvelopeStorage/>
        </>
    );
}

export default AdminManageEnvelope;