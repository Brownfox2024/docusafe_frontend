...
import {
    adminGetTemplateDocumentData,
    adminGetTemplateList, adminPostUseTemplateDocumentData
} from "../../../../../services/AdminService";

function AdminDocFormTemplate(props) {
    ...
    const clsUseFromTemplateRef = useRef(null);

    useEffect(function () {
        if (props.isUseFromTemplate) {
            setLoading(true);
            let obj = {
                client_id: client,
    ...
    }, [props, currentTemplate, client, selectedTemplateList]);
    ...
    const handleCloseUseFromTemplate = (e) => {
        e.preventDefault();
        props.setIsUseFromTemplate(false);
        props.setFromTemplate(1);
        setIsBack(-1);
        setTemplateList([]);
        setBreadcrumbs([]);
    ...
    const onSelectTemplate = (e, data, index) => {
        ...
        adminGetTemplateDocumentData(obj)
            .then(response => {
    ...
    const handleDocFormSave = (e) => {
        ...
        if (selectedTemplateList.length > 0) {
            ...
            if (docIds.length > 0 || dataIds.length > 0) {
                ...
                let obj = {
                    client_id: client,
                    id: props.formObj.envelope_id,
                    doc_ids: docIds,
                    data_ids: dataIds
                };
                adminPostUseTemplateDocumentData(obj)
                    .then(response => {
                        clsUseFromTemplateRef?.current.click();
                        setLoading(false);
                        let formData = {...props.formObj};
                        let docList = formData.documents;
                        formData.documents = [...docList, ...response.data.docList];
                        let fList = formData.fill_forms;
                        formData.fill_forms = [...fList, ...response.data.dataList];
    ...
    return (
        <>
            ...
            <div className="offcanvas offcanvas-end AddDocument AddTemplate templates_page" id="AddDocFormTemplate"
                 data-bs-backdrop="static" data-bs-keyboard="false">
                <div className="offcanvas-header">
                    <h5 className="offcanvas-title" id="AddDocFormTemplateLabel">Add From Templates</h5>
                    <button type="button" className="btn close_btn text-reset" data-bs-dismiss="offcanvas"
                            ref={clsUseFromTemplateRef} onClick={handleCloseUseFromTemplate} aria-label="Close">
                ...
                <div className="offcanvas-body p-0 " style={{overflowX: 'hidden'}}>
                    ...
                    <div className="modal-footer  mb-0 ">
                        <button type="button" onClick={handleCloseUseFromTemplate} className="btn grey_btn_outline"
                                data-bs-dismiss="offcanvas">Cancel
                        </button>
                        <button type="button" onClick={handleDocFormSave} className="btn modal_btn">Save</button>
                    </div>
                </div>
            </div>
        </>
...
export default AdminDocFormTemplate;