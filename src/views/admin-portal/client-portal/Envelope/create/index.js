...
import AdminClientEnvelopeStep1 from "./Step1";
import AdminClientEnvelopeStep2 from "./Step2";
import AdminClientEnvelopeStep3 from "./Step3";
import AdminClientEnvelopeStep4 from "./Step4";
import {useParams} from 'react-router-dom';
import {toast} from "react-toastify";
import Utils from "../../../../../utils";
import AdminEnvelopeStepWizard from "../../partials/AdminEnvelopeStepWizard";
import {
    adminCheckTemplatePost,
    adminGetEnvelopeDetail,
    adminGetEnvelopeSenderList,
    adminGetPreferenceData, adminPostCloudList
} from "../../../../../services/AdminService";

function AdminClientEnvelopeCreate() {
    const [loading, setLoading] = useState(false);
    const [senderList, setSenderList] = useState([]);
    const [isCall, setIsCall] = useState(false);
    const [isTemplate, setIsTemplate] = useState(false);
    const {client, uuid, id, direct} = useParams();
        ...
        if (uuid && !envelopeData.envelope_uuid) {
            ...
            adminPostCloudList({client_id: client})
                .then(response => {
                    ...
                    adminGetEnvelopeDetail({client_id: client, uuid: uuid})
                        .then(response => {
    ...
    }, [client, uuid, formObj, direct, isCall]);

    const renderStep = (step) => {
        if (parseInt(step) === 1) {
            return <AdminClientEnvelopeStep1 senderList={senderList} formObj={formObj} setFormObj={setFormObj}
                                             setLoading={setLoading}/>

        } else if (parseInt(step) === 2) {
            return <AdminClientEnvelopeStep2 formObj={formObj} setFormObj={setFormObj} setLoading={setLoading}/>

        } else if (parseInt(step) === 3) {
            return <AdminClientEnvelopeStep3 senderList={senderList} formObj={formObj} setFormObj={setFormObj}
                                             setLoading={setLoading} isTemplate={isTemplate}/>

        } else if (parseInt(step) === 4) {
            return <AdminClientEnvelopeStep4 senderList={senderList} formObj={formObj} setFormObj={setFormObj}
                                             setLoading={setLoading} isTemplate={isTemplate}/>
        }
    };
    ...
    return (
        <>
            ...
            <section className="main_wrapper background_grey_400 detail_page"
                ...
                <AdminEnvelopeStepWizard step={formObj.current_step} isDone={formObj.is_done}/>
...
export default AdminClientEnvelopeCreate;