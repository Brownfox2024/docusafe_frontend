...
import {adminMoveEnvelopeTemplate, adminTemplateFolderList} from "../../../../../services/AdminService";

function AdminMoveEnvelope(props) {
    ...
    useEffect(function () {
        if (props?.folderData?.id > 0) {
            adminTemplateFolderList({client_id: client, is_all: 1, folder_id: props.folderData.id})
                .then(response => {
...
export default AdminMoveEnvelope;