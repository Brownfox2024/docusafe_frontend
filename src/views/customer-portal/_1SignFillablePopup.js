...
function SignFillablePopup({
  ...
  const removeObjectsByFilter = (filterCondition) => {
  ...
  const handleImageClick = (index) => {
    ...
    setTimeout(() => {
      ...
      const items =
        placeholders !== null
          ? placeholdersArr.filter((item) => item.page_object_id === page)
          : [];
      if (items.length > 0) {
        for (let j = 0; j < items.length; j++) {
              ...
              <div className="modal-content">
                ...
                <div className="modal-footer">
                    ...
                    {/* <div className="d-flex justify-content-between align-items-center w-100">
                      <div>
                        <input type="checkbox" id="apply-everywhere" name="apply-everywhere" value="1" />
                        <label for="apply-everywhere"> Apply everywhere</label>
                      </div>
                      <div>
                          <button type="button" onClick={() => setShowSignModal(false)} className="btn btn-secondary" data-dismiss="modal">Close</button>
                          <button type="button" onClick={() => applySign()} className="btn btn-primary">Save</button>
                      </div>
                    </div> */}
                </div>
              </div>
