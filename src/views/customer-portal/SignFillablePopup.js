...
function SignFillablePopup({
  ...
  */
  ...
  }, [signFillRequest, imageList, canvas, pdfDimensions]);
  ...
  function getCurrentRecipientName() {
    const firstName = formObj.recipient_detail.first_name;
    const lastName = formObj.recipient_detail.last_name;
  ...
  function createRectangle(objId, imageWidth, imageHeight, fillColor) {
    ...
    //   rectWidth = (textLength * 7.5);
    // }else if (objId.indexOf("initial-textbox") > -1){
    //   rectWidth = (textLength * 6.8);
    // }else{
    //   if(textLength > 40){
    //     rectWidth = (textLength * 7);  
    //   }else{
    //     rectWidth = (textLength * 9.5);
    //   }
    // }
    var rectWidth = imageWidth + 30;
    var rectHeight = imageHeight + 26;
  ...
  function fabricTool(
    ...
    createImage(imageURL)
      .then((img) => {
        ...
        let fTextLabelWidth = "";
        if(fontFamily === "Arial"){
          fTextLabelWidth = textContent.length * 7.5;
  ...
  const scrollToPage = (pageNumber) => {
    ...
    var dynamicHeight = (pageNumber * pdfHeight);
  ...
  const saveDecline = (e) => {
    if (declineTextareaValue === "") {
      toast.error("Please enter the decline reason.");
      return false;
    }
    ...
    setLoading(true);    
    declineCustomerSignDocument(obj)
      .then((response) => {
        /*
