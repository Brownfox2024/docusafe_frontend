...
import {getEnvelopeExpireDocuments, sendEnvelopeExpireDocuments} from "../../../../../services/CommonService";
...
function ExpiredDocument() {
    ...
    useEffect(function () {
        ...
        let obj = {id: id};
        getEnvelopeExpireDocuments(obj)
            .then(response => {
    ...
    const handleSendEnvelope = (e) => {
        ...
        if (recipients.length > 0) {
            ...
            sendEnvelopeExpireDocuments(obj)
                .then(response => {
                    setLoading(false);
                    setIsInvited(1);
                })
    ...
    function onViewEnvelope(e) {
        e.preventDefault();
        navigate("/manage/" + envelopeData.uuid);
    }
