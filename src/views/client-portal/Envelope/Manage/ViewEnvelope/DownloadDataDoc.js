...
import {
    envelopeDownloadAllFile,
    userEnvelopeSpecificDownloadData
...
function DownloadDataDoc(props) {
    ...
    const downloadSpecificSingleFile = (e, index, idx) => {
        ...
        for (let i = 0; i < recipientList?.length; i++) {
            ...
            if (documents.includes(recipientobj)) {
                ...
                } else if (item.entity_type === 3) {
                    sign_doc_list.push(parseInt(item.id));
                }

                obj['doc_ids'] = doc_list;
                obj['form_ids'] = form_list;
                obj['sign_doc_ids'] = sign_doc_list;
            }
        ...
        let obj = {
            envelope_id: parseInt(envId),
            recipients: totalRecipient
        };

        props.setLoading(true);

        userEnvelopeSpecificDownloadData(obj)
            .then(result => {
            ...
            if (checked) {
                ...
                for (let k = 0; k < documents.length; k++) {
                    ...
                    if (entity_type === 1) {
                        ...
                        if (checked) {
                            sign_doc_list.push(parseInt(id));
                        }
                    }
                }
                obj['doc_ids'] = doc_list;
...
export default DownloadDataDoc;