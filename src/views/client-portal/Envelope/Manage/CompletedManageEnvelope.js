...
import {
    ...
    getManageEnvelopeList, userEnvelopeDownloadData
...
import DownloadDataDoc from "./ViewEnvelope/DownloadDataDoc";

const lengths = Utils.tableShowLengths();

function CompletedManageEnvelope() {
    ...
    const handleExpired = (e, data) => {
        ...
        navigate('/manage/' + data.uuid + '/expired-document');
    };
                                ...
                                <table
                                    ...
                                    {envelopeList.map((item, index) =>
                                        <tr key={index}>
                                            ...
                                            <td className="text-center">
                                                {item.cloud_sync.split(",").map(Number).some(value => value > 0) && item.cloud_type.split(",").map(Number).some(value => value > 0) > 0 && (
                                                    <i className="fa fa-cloud-upload fontSize text-primary"
                                                       data-toggle="tooltip" onClick={(e) => handleSendCloud(e, item)}
                                                       data-placement="right" title=""
...
export default CompletedManageEnvelope;