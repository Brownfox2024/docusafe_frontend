...
import {modifyEnvelopeRequestInformation} from "../../../../services/CommonService";
...
function EnvelopeRequestForm(props) {
    ...
    }, [props?.requestFormData, props.formObj]);
    ...
    function onAddFieldDate(e, index) {
        e.preventDefault();
    }
    ...
    function storeFormField(e, type) {
        ...
        modifyEnvelopeRequestInformation(obj)
            .then(response => {
                if (type === 1) {
                    clsFormButtonRef.current?.click();
                    clearForm(e);
                ...
                if (formId > 0) {
                    let index = envelopeFormData.fill_forms.findIndex(x => parseInt(x.id) === parseInt(formId));
                    if (index > -1) {
                        envelopeFormData.fill_forms[index]['name'] = formName;
                        envelopeFormData.fill_forms[index]['request_id'] = recipientIds;
                        envelopeFormData.fill_forms[index]['request_display'] = requestDisplay;
                        envelopeFormData.fill_forms[index]['questions'] = formQuestions;
                    }
                } else {
                    envelopeFormData.fill_forms.push({
                        id: response.data.id,
                        name: formName,
    ...
    return (
        <div className="offcanvas offcanvas-end AddDocument MakeForm" data-bs-scroll="true" data-bs-backdrop="static"
             data-bs-keyboard="false" tabIndex="-1" id="MakeForm" aria-labelledby="MakeFormLabel">
            <div className="offcanvas-header">
                <h5 className="offcanvas-title"
                    id="MakeFormLabel">{formId > 0 ? `Edit Form` : `Create Form`}</h5>
            ...
            <div className="offcanvas-body p-0 " style={{overflowX: 'hidden'}}>
                <div className="row">
                    <div className="col-lg-3 py-0 border-end">
                        <ul className="left_canvas ">
                        ...
                        <div className="accordion ps-2" id="">
                            ...
                            <div className="accordion-item bg-blue mb-2 ">
                        ...
                        <div className="accordion ps_sm_4 pe-4 mt-3" id="AdditionalField">
                            {previewContent === "" ? (
                                ...
                                {formQuestions.map((item, index) =>
                                    // <div key={index}  onDragStart={(e) => questionDragStart(e, index)} draggable
                                    // onDragEnter={(e) => questionDragEnter(e, index)} onDragEnd={questionDrop} className="d-flex align-items-center pe-3 box-item" >
                                    <div key={index} className="d-flex align-items-center pe-3 box-item" >
                                        <div className="card my-3 w-100">
                                            <div className={`d-flex align-items-center mb-2 ${item.type === 7 ? 'pt-3' : ''}`}>
                                                ...
                                                src="/images/dots_rectangle.png" alt="img" className="me-3 drag-img"/>
                                                {item.type !== 7 && (
                                                ...
                                                <div className="">
                                                    <div className="position-relative">
                                                        <DatePicker
                                                            ...
                                                            className="form-control"
                                                            placeholderText={showDatePlaceholder(2)}
                                                            onChange={(e) => onAddFieldDate(e, index)} disabled/>
                                                        <i className="fa fa-calendar" style={{top: "15px"}}
                                                        onClick={(e) => dateRef?.current.setFocus(true)}/>
                                                    </div>
                                                </div>
                                            ...
                                            {item.type === 7 && (
                                                ...
                                                //             'heading', '|',
                                                //             'bold', 'italic', '|',
                                                //             'link', '|',  // Add 'link' option to include anchor links
                                                //             'bulletedList', 'numberedList', '|',
                                                //             'undo', 'redo'
                                                //         ],
                                                //         link: {
                                                //             addTargetToExternalLinks: true  // Enable adding target="_blank" to external links
                                                //         }
                                                //     }}
                                                //     onReady={editor => {
                                                ...
                                                //             toolbarButtons: ['bold', 'italic', 'underline', '|', 'fontFamily', 'fontSize', 'color', 'inlineStyle', '|', 'align', 'formatOL', 'formatUL', '|', 'insertLink', '|', 'undo', 'redo'],
                                                //             height: 200,
                                                //             quickInsertEnabled: false,
                                                //             events: {
                                                //                 'froalaEditor.contentChanged': (e, editor) => handleModelChange(editor.html.get(), index)
                                                //             }
                                                //         }}
                                                //         model={item.select_options}
...
export default EnvelopeRequestForm;